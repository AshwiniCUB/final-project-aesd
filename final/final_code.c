...
#define FILE_PATH "/etc/tmpdir/sensordata.txt"
...
void appendSensorDataToFile(double temperature, double pressure, double humidity) {
    ...
    fprintf(file, "Temperature: %.2f°C, Station Pressure: %.2f hPa, Humidity: %.2f %%rH\n",
...
void readSensorData(int file_descriptor) {
    ...
    double compensatedTemperature = 0.0;
    double stationPressure = 0.0;
    double compensatedHumidity = 0.0;
    ...
    while (1) {
        ...
        compensatedTemperature = compensateTemperature(temp_raw);
        stationPressure = compensatePressure(pressure_raw) / 100.0;
        compensatedHumidity = compensateHumidity(humidity_raw);

        // Append sensor data to the file
        appendSensorDataToFile(compensatedTemperature, stationPressure, compensatedHumidity);

        // Read and print sensor data from the file
